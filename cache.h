...
public:
	vector3 get_bone_3d(int id) const {
		...
		auto bone_transform = read<FTransform>(read<u>((u)this + 0x10 * is_cached + 0x5B0) + 0x60 * id);
...
public:
	...
	ugameinstance* get_game_instance() const {
		SPOOF_FUNC
			if (!(u)this) return 0;
		return read<ugameinstance*>((u)this + 0x1D8);
	}
	agamestate* get_game_state() const {
		SPOOF_FUNC
			if (!(u)this) return 0;
		return read<agamestate*>((u)this + 0x160);
	}
};
...
bool populate_global_data(const std::shared_ptr<global_s>& global_temp) {
	...
	global_temp->world = read<uworld*>(mem::BaseAddy + 0x124BF388);
