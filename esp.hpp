...
void update_camera()
{
	if (auto LocalPlayerCachedLODDistanceFactor = read<unsigned long long>((u)global->local_player + 0xd0))
	{
		if (LocalPlayerCachedLODDistanceFactor = read<unsigned long long>(LocalPlayerCachedLODDistanceFactor + 0x8))
		{
			uint64_t FGC_Pointerloc = read<uint64_t>((u)global->world + 0x110);
			camera::location = read<vector3>(FGC_Pointerloc);
			auto YawSine = read<double>(LocalPlayerCachedLODDistanceFactor + 0x8f0);
			auto YawCosine = read<double>(LocalPlayerCachedLODDistanceFactor + 0x910);

			camera::rotation.x = asin(read<double>(LocalPlayerCachedLODDistanceFactor + 0x9c0)) * (180.0 / 3.14159265358979323846);
			camera::rotation.y = atan2(YawSine, YawCosine) * (180.0 / 3.14159265358979323846) * -1;
			camera::rotation.z = 0;
			camera::fov = read<float>((u)global->player_controller + /*0x38C*/ 0x2a4) * 90.f;
		}
	}
}
...
void actor_loop() {
	...
	if (draw_fov) {
		...
		else
		{
			...
			{
				draw_list->AddCircle(ImVec2(screen_width / 2, screen_height / 2), dynamicfov, ImColor(0, 0, 0, 255), 100, 2);
			}
