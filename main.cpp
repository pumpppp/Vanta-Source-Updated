...
inline void DrawMenu() {
	...
	if (menu_open)
	{
		...
		if (MenuTab == 1)
		{
			if (customcolor)
			{
				if (ImGui::CoolButton(ImColor(Buttomshi), TEXT("Aim Assistance"), ImVec2(120, 20)))
				{
					MenuTab = 1;
				}
			...
			{
				if (ImGui::CoolButton(ImVec4(0.65f, 0.05f, 0.9f, 1.0f), TEXT("Aim Assistance"), ImVec2(120, 20)))
				{
					MenuTab = 1;
				}
			...
			ImGui::CoolCheckbox("  Enable Aimbot  ", &aimbot);
			if (aimbot)
			{
				memory = false;
			}
			ImGui::CoolCheckbox("  Enable Memory Aim ", &memory);
			if (memory)
			{
				aimbot = false;
			}
		...
		if (MenuTab == 8)
		{
			...
			{
				if (ImGui::CoolButton(ImColor(19, 19, 19), TEXT("Aim Assistance"), ImVec2(120, 20)))
				{
					MenuTab = 1;
				}
		...
		if (MenuTab == 11)
		{
			...
			if (WeaponCfgs)
			{
				...
				ImGui::CoolCheckbox("  Enable Memory Aim", &sniper_memory);
				if (sniper_memory)
				{
					sniper_aimbot = false;
				}
		...
		//top text
		{
			...
			if (draw_username) {
				auto Size = ImGui::CalcTextSize(TEXT("Pump"));
				draw_list->AddText(ImVec2(__top_x - (Size.x / 2), __top_y - ThisOffset1), ImGui::GetColorU32({ ColorTopText.x, ColorTopText.y, ColorTopText.z, 1.0f }), TEXT("Pump"));
			}
